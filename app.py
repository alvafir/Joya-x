...
from modules.market_orchestrator import run_decision_engine
from modules.scanner import scan_fixtures

st.set_page_config(
    page_title="JOYA Enterprise 4.3",
    page_icon="🏆",
    layout="wide",
)

st.title("🏆 JOYA Enterprise 4.3")
st.caption("v4.3.0 DEV · JOYA Decision Engine")
        ...
        if match_table.empty:
            ...
            if st.button(
                ...
                if not api_prediction["ok"]:
                    ...
                    league_score = (
                        int(league_row.iloc[0]["League Score"])
                    ...
                    st.session_state["decision_markets"] = decisions
...
decision_markets = st.session_state.get("decision_markets")

if decision_markets:
    st.divider()
    st.subheader("🧠 JOYA Decision Engine")

    approved = [
        market for market in decision_markets
        if market["status"] == "🟢 BET"
    ]

    caution = [
        market for market in decision_markets
        if market["status"] == "🟡 PRECAUCIÓN"
    ]

    d1, d2, d3 = st.columns(3)
    d1.metric("Mercados analizados", len(decision_markets))
    d2.metric("BET", len(approved))
    d3.metric("PRECAUCIÓN", len(caution))

    show_only_safe = st.checkbox(
        "Mostrar solo BET y PRECAUCIÓN",
        value=True,
    )
    ...
    visible_markets = (
        [
            market for market in decision_markets
            if market["status"] != "🔴 NO BET"
        ]
        if show_only_safe
        else decision_markets
    )

    if not visible_markets:
        st.warning(
            "No hay mercados aprobados con la evidencia disponible."
        )
    else:
        import pandas as pd

        frame = pd.DataFrame(visible_markets)
        frame = frame[
            [
                "family",
                "market",
            ...
            columns={
                ...
                "risk": "Riesgo",
                "sample_quality": "Calidad muestra",
        ...
        st.caption(
            "Corners, tarjetas, remates y mercados temporales quedan NO BET "
            "cuando faltan estadísticas específicas. JOYA no inventa datos."
        )
