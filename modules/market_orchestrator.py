...
def evaluate_all(projection, quality, league_score):
    return {
...
def _correlation_group(market: str) -> str:
    ...
    if "rango" in m: return "goal_range"
    if "ambos marcan" in m: return "btts"
    if "goles 1t" in m: return "goals_1h"
    return m

def global_ranking(engines: Dict[str, List[MarketDecision]], limit: int = 20) -> List[MarketDecision]:
    candidates = sorted(
