...
def get_fixture_prediction(fixture_id: int) -> dict[str, Any]:
    """
    Obtiene la predicción oficial disponible en API-Football
    para un fixture concreto.
    """
    try:
        response = requests.get(
    ...
    if not response_items:
        return {
            "ok": False,
            "message": "No hay predicción disponible para este partido.",
            "data": None,
        }

    return {
        "ok": True,
        "message": "Predicción cargada.",
        "data": response_items[0],
    }
