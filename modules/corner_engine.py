...
def evaluate(
    ...
    if h2h_report and h2h_report.available and h2h_report.avg_corners is not None:
        h2h_weight = min(0.14, 0.14 * h2h_report.score / 100)
        cp.total_expected = round(
            cp.total_expected * (1 - h2h_weight)
            + h2h_report.avg_corners * h2h_weight,
            2,
        )
    pc = cp.confidence
